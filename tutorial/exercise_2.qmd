...
`requirements.txt` files — create a file named `Dockerfile` (no extension) with the following 
contents:
...
2. Build the image using the following commands:
   ...
   already on your machine, Docker will fetch it first. This may take some time.
   ...
   and defines how to start the app with `CMD ["python", "app.py"]` (your 'entrypoint').
...
image you just created:
   ...
   ```{bash}
   docker run -d -p 5001:5000 loud-text
   ```
...
In part 1, you containerized a simple web app. In this part, you'll update the 
application and image. You'll also learn how to stop and remove a container.

## Update the source code

In the following steps, you'll change the "placeholder" when you haven't typed 
anything into the application yet to "Yell something...".

1. In the `app.py` file, update line 62 to use the new empty text.

```diff
-       <input type="text" name="text" placeholder="Type something..." autofocus>
+       <input type="text" name="text" placeholder="Yell something..." autofocus>
```
...
```{bash}
docker run -dp 5001:5000 loud-text
```

You probably saw an error like this:

```{console}
docker: Error response from daemon: driver failed programming external connectivity on endpoint sweet_yonath (5ecf0c721cf409e9cf7202e19a2049bbce7cd8d904d2800a53febb5d1d8f0cc6): Bind for 0.0.0.0:5001 failed: port is already allocated.
```

The error occurred because you aren't able to start the new container while your
old container is still running. The reason is that the old container is already 
using the host's port 5001 and only one process on the machine (containers included)
can listen to a specific port. To fix this, you need to remove the old container.

## Remove the old container

To remove a container, you first need to stop it. Once it has stopped, you can 
remove it. You can remove the old container using the CLI or Docker Desktop's 
graphical interface. Choose the option that you're most comfortable with.
...
```{bash}
docker run -dp 127.0.0.1:5001:5000 loud-text
```

2. Refresh your browser on [http://localhost:5001](http://localhost:5001) and you should see your updated placeholder text.
...
Next, you're going to learn how to mount your local directory to the docker 
container and how to set environment variables. By using these features, you will decode an encrypted message.
